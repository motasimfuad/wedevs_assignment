import 'package:flutter/material.dart';

class AppColors {
  static const Color primary = Color(0xFFF75F55);
  static const Color secondary = Color(0xFF6E7FAA);
  static const Color pink = Color(0xFFFF679B);
  static const Color lightPink = Color(0xFFFFD3DD);
  static const Color success = Color(0xFF1ABC9C);
  static const Color grey = Color(0xFF989FA8);
  static const Color darkGrey = Color(0xFF818995);
  static const Color yellow = Color(0xFFF5A623);
  static const Color pageBackground = Color(0xFFF8F8F8);
  static const Color pageBackgroundLighter = Color(0xFFF8F8FA);

  /// Gradient
  static const List<Color> primaryGradient = [
    pink,
    primary,
  ];
}
